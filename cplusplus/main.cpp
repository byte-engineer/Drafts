// INFO----------------------------------------------------
// - C/C++ is a very performent general-purpose programming language, staticly typed, compiled language , has C-like sytax.
// - C++ reserves a set of 92 words (keywords) (as of C++23).   // https://www.learncpp.com/cpp-tutorial/keywords-and-naming-identifiers/
// - C   reserves a set of 32 keywords.

/* Running the script--------------------------------------
 * - Download the compiler (gcc / clang).
 ...
 * - Run generated binary file. `./a.out`
 */
...
// standard input and output library in cpp (std).

// Without `_`.                                                 // I wrote the underScore for technical reasons.
int _main(void)
{ // From here our code will start executing ( our entry point )
    // Put here a block of code.
    return 0; // Returning from the main function 0 means that the program finished executed.
}
...
// C++ is a staticly typed language so we need to inculde the type of the varible in declerations.
// We have many different types of variables. {int, double,...}
// We cannot use same variable name for different variables (No shadowing).
...
void Variables()
{
    ...
    // Some Primitive Variables Types.                                                                 |_Size_____|_Range_________________
    char ltr = 'A';       // For store a single letter or number.| 1 byte   | ±128         | 0-255 for unsigned char
    ...
    int year1 = age++; // year will not be assigned to the incremented value
...
// We use cin to take user input.
// This two classes are exist in iosream header.
// `cout` is an instance from ostream class.
// `cin` is an instance from istream class.
// Almost all primitvs are implemnted ostream.
// For user defined types we need to implement ostream our selves.

#include <iostream> // cout, cin, ... lives here.
...
// implement ostream for point so we can print it.
std::ostream &operator<<(std::ostream &ostrm, point p)
{
...
void standardIO()
{
    ...
    std::cin >> var1 >> var2; // User can enter values seperated with white spaces {' ', '\n', '\t'}

    if (std::cin >> var1)
    {
    } // If there are any errors it will return false.
    ...
    // std::endl                                                // Write a newline and output flush the stream.
    // std::flush                                               // Flush output stream.
}
...
// There is implicit name space defined by the language which is the global name space.
// Almost all standard librabry functinality used to inside `std` name space.
...
void NameSpaces()
{
    // using some thing inside a name space.
    // technecly called "Explicit namespace qualifier"
    std::cout; // calling cout from std namespaces.
    fruits::apple;
    ...
    apple; // Accesing apple without fruits:: prefix.
}

// preprocessor------------------------------------------------------
// When the preprocessor has finished processing a code file, the result is called a translation unit.
// "Preprocessor directives" are instructions that start with a # symbol and end with a newline (NOT a semicolon).
...
void ifStatement()
{
    ...
    // Normal If statement.
    if (condition)
    { // We can get rid 0f the curly barkcets (if our code in one line) and Will work just fine.
      // ... do something
    }
    ...
    { // Actually we don't have any 'if else' saperate keyword! it's just a compination between 'if' keyword and 'else' keyword.
      // Do some thing else.
    }
    else
    {
...
void loops()
{
    ...
    {
    } // For loop in an iterator like vectors, strings,array, ...
    for (auto iter = iterator.begin(); iter != iterator.end(); ++iter)
    {
    } // This what is happaning BTS.
    for (auto iter = iterator.rbegin(); iter != iterator.rend(); ++iter)
    {
    } // This looping backword.
...
{
public:
    std::string firstName;                       // This the class atributes ( Variables )
